#!/usr/bin/env python3
"""
Motorola 68020 CPI Stack Model
...
- Prefetch penalty: Stalls waiting for instruction fetch
- Branch penalty: Pipeline flush on taken branches
- Memory penalty: Extra cycles for memory operands
- Cache miss penalty: Cycles lost to cache misses
...
class CPIStackResult:
    ...
    def compute_totals(self):
        self.cpi_total = self.cpi_base+ self.cpi_prefetch+ self.cpi_branch + self.cpi_memory+ self.cpi_cache+ self.cpi_pipeline
        self.ipc = 1.0 / self.cpi_total if self.cpi_total > 0 else 0
        self.mips = 16.0 * self.ipc
...
class Motorola68020CPIStackModel:
    """
    CPI Stack Model for Motorola 68020.
    ...
    def predict(self, workload: Workload = None) -> CPIStackResult:
        ...
        # Prefetch penalty
        result.cpi_prefetch = result.cpi_base * max(0, (6 - self.PREFETCH_BYTES) * 0.05)
        
        # Branch penalty
        taken = workload.mix_branch * workload.branch_taken_rate
        result.cpi_branch = taken * self.BRANCH_PENALTY
        ...
        # Cache miss penalty
        miss_rate = 1 - self.CACHE_HIT_RATE
        result.cpi_cache = workload.mix_memory * miss_rate * 10
        
        # Pipeline hazard penalty
        result.cpi_pipeline = result.cpi_base * 0.02 * (3 - 2)
    ...
    def print_result(self, result: CPIStackResult):
        ...
        print(f"\n{'='*55}")
        print(f"CPI STACK: Motorola 68020")
        print(f"{'='*55}")
...
def main():
    model = Motorola68020CPIStackModel()
    
    print("\n" + "="*55)
    print("Motorola 68020 CPI STACK MODEL")
    print("="*55)
