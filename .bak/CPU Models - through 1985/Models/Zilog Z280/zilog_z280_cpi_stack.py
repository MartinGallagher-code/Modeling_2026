...
class CPIStackResult:
    ...
    def compute_totals(self):
        self.cpi_total = self.cpi_base+ self.cpi_prefetch+ self.cpi_branch + self.cpi_memory+ self.cpi_cache
        self.ipc = 1.0 / self.cpi_total if self.cpi_total > 0 else 0
        self.mips = 12.0 * self.ipc
...
class ZilogZ280CPIStackModel:
    """
    CPI Stack Model for Zilog Z280.
...
def main():
    model = ZilogZ280CPIStackModel()
    
    print("\n" + "="*55)
    print("Zilog Z280 CPI STACK MODEL")
    print("="*55)
