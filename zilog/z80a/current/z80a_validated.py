#!/usr/bin/env python3
"""
Zilog Z80A Validated Performance Model
...
@dataclass
class Z80ASpecs:
    """Validated Zilog Z80A specifications."""
    name: str = "Zilog Z80A"
    year: int = 1978
...
class Z80AModel:
    """
    Validated performance model for Zilog Z80A.
    
    Uses grey-box queueing methodology with category-based timing.
    """
    
    def __init__(self):
        self.specs = Z80ASpecs()
        self.workloads = {
            'typical': Z80AWorkload('typical', 0.3, 0.5, 0.2),
            'compute': Z80AWorkload('compute', 0.5, 0.4, 0.1),
            'control': Z80AWorkload('control', 0.2, 0.4, 0.4),
        }
    
    def analyze(self, workload: str = 'typical',
                clock_mhz: float = 4.0) -> Z80AResult:
