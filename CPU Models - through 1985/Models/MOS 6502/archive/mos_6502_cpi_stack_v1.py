#!/usr/bin/env python3
"""
MOS 6502 CPI Stack Model - ORIGINAL VERSION (ARCHIVED)

================================================================================
DEPRECATED: This is the original simplified model retained for historical
comparison. For validated, accurate modeling, use:

    from mos_6502_validated import MOS6502Model

See comparison below:

    Metric      | Original | Validated | Expected
    ------------|----------|-----------|----------
    CPI         | ~3.0     | 3.216     | 2.3-3.2
    IPC         | ~0.33    | 0.311     | 0.31-0.43
    MIPS @ 1MHz | ~0.33    | 0.311     | 0.31-0.43

The validated model includes:
- Complete instruction timing database (56 instructions)
...
Status: ARCHIVED - See mos_6502_validated.py for current version
"""
...
warnings.warn(
    "mos_6502_cpi_stack_v1.py is deprecated. "
    "Use mos_6502_validated.py for accurate, validated modeling.",
    DeprecationWarning,
    stacklevel=2
)
...
class Mos6502CPIStackModel:
    """
    CPI Stack Model for MOS 6502 (ORIGINAL - SIMPLIFIED).
    
    DEPRECATED: Use MOS6502Model from mos_6502_validated.py instead.
    
    This simplified model uses fixed timing values that don't account for:
    ...
    # SIMPLIFIED instruction timings (cycles)
    # Note: These are approximations, not validated values
    TIMING_ALU = 2      # Actual: varies 2-6 by addressing mode
    TIMING_MOV = 3      # Actual: varies 2-6 by addressing mode
    TIMING_BRANCH = 3   # Actual: 2 (not taken), 3 (taken), 4 (page cross)
    TIMING_MEMORY = 4   # Actual: varies 3-7 by addressing mode
    ...
    def print_result(self, result: CPIStackResult):
        ...
        print(f"\n{'='*55}")
        print(f"CPI STACK: MOS 6502 (ORIGINAL - ARCHIVED)")
        print(f"{'='*55}")
...
def main():
    print("\n" + "="*60)
    print("MOS 6502 CPI STACK MODEL - ORIGINAL VERSION (ARCHIVED)")
    print("="*60)
    print("⚠️  DEPRECATED: Use mos_6502_validated.py for accurate results")
    print("="*60)
    
    model = Mos6502CPIStackModel()
