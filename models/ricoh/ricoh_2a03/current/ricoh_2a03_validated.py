#!/usr/bin/env python3
"""
Ricoh 2A03 Grey-Box Queueing Model
===================================
...
Features:
  - 6502 core WITHOUT decimal mode (BCD disabled)
  - 8-bit data bus, 16-bit address bus
  - Runs at 1.79 MHz (NTSC) or 1.66 MHz (PAL)
  - Includes APU (audio processing unit) - not modeled here
  - Same instruction timing as MOS 6502
  - 2-7 cycles per instruction

Calibrated: 2026-01-29
Target CPI: ~3.0 for typical workloads (cross-validated with MOS 6502)
Used in: Nintendo Entertainment System (NES), Famicom
"""
...
class Ricoh2A03Model(BaseProcessorModel):
    """
    Ricoh 2A03 Grey-Box Queueing Model

    Architecture: 8-bit NMOS microprocessor (1983)
    - 6502 core with BCD (decimal mode) disabled
    - Sequential execution (no pipeline)
    - Efficient addressing modes (zero-page is key)
    - 2-7 cycles per instruction
    - CPI ~3.0 for typical workloads (identical to MOS 6502)

    Key difference from MOS 6502:
    - SED (Set Decimal) instruction exists but has no effect
    - ADC/SBC always operate in binary mode
    - Includes on-chip APU for audio (not modeled here)
    """
    ...
    manufacturer = "Ricoh"
    year = 1983
    clock_mhz = 1.79  # NTSC clock (PAL variant 2A07 runs at 1.66 MHz)
    transistor_count = 3510  # Same as 6502 core (excludes APU)
    data_width = 8
    address_width = 16

    def __init__(self):
        # 2A03 instruction timing - identical to MOS 6502
        # From MOS Technology datasheet and FCEUX/Nestopia emulator validation
        #
        # Actual instruction timings (same as 6502):
        ...
        #
        # Note: BCD mode disabled, but ADC/SBC timing unchanged

        # Instruction categories calibrated via cross-validation against
        # MOS 6502 datasheet timings and realistic NES instruction mix analysis
        # Target CPI: ~3.0 (validated against actual NES software)
        ...
        # Workload profiles based on NES game analysis
        # NES games are typically heavy on control flow and memory access
        self.workload_profiles = {
            'typical': WorkloadProfile('typical', {
                ...
                'control': 0.20,
                'stack': 0.10,
            }, "Typical NES workload (games, system software)"),
            'compute': WorkloadProfile('compute', {
                ...
                'control': 0.15,
                'stack': 0.05,
            }, "Compute-intensive (math routines, physics)"),
            'memory': WorkloadProfile('memory', {
                ...
                'control': 0.12,
                'stack': 0.08,
            }, "Memory-intensive (graphics updates, DMA)"),
            'control': WorkloadProfile('control', {
                ...
                'control': 0.35,
                'stack': 0.15,
            }, "Control-flow intensive (game logic, AI)"),
        }
    ...
    def validate(self) -> Dict[str, Any]:
        """Run validation tests against known 2A03/6502 characteristics"""
        tests = []

        # Test 1: CPI within expected range (target 3.0, cross-validated with 6502)
        ...
        # Test 3: All cycle counts are positive and reasonable (1-10 for 6502/2A03)
        ...
        # Test 4: IPC is in valid range for 2A03 (0.2 - 0.5 typical)
        tests.append({
            'name': 'IPC range',
        ...
        # Test 6: Cross-validation with MOS 6502 timing
        # The 2A03 should have identical CPI to 6502
        tests.append({
            'name': '6502 cross-validation',
            'passed': 2.9 <= result.cpi <= 3.1,
            'expected': '2.9-3.1 (matches MOS 6502)',
            'actual': f'{result.cpi:.2f}'
        })

        # Test 7: Clock speed validation (NTSC)
        tests.append({
            'name': 'Clock speed (NTSC)',
            'passed': abs(self.clock_mhz - 1.79) < 0.01,
            'expected': '1.79 MHz',
